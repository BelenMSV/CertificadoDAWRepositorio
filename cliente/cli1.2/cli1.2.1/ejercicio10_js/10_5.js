...
function eliminar() {
   ...
   let pos = buscar(dts, elemento);
   if(pos != -1){
      let dl = document.getElementsByTagName("dl")[0];
      eliminarUno(dl, "dd", pos);
      eliminarUno(dl, "dt", pos);
   }else{
      alert("La definición que quieres eliminar no existe");
   }
...
const buscar = (array, elemento) =>{
   ...
   while(!encontrado && i<array.length){
      console.log(array[i].innerText.toUpperCase() );
         console.log(elemento.toUpperCase() );
       if(array[i].innerText.toUpperCase() == elemento.toUpperCase()){
